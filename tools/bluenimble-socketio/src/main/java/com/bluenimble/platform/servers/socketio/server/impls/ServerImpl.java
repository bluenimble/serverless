package com.bluenimble.platform.servers.socketio.server.impls;

import java.io.File;
import java.io.FileInputStream;

import com.bluenimble.platform.servers.socketio.listeners.OnConnectListener;
import com.bluenimble.platform.servers.socketio.listeners.OnJoinListener;
import com.bluenimble.platform.servers.socketio.listeners.OnLeaveListener;
...
public class ServerImpl implements Server {
	...
	public ServerImpl (JsonObject spec) throws Exception {
		...
		if (!Json.isNullOrEmpty (ssl)) {
			...
			config.setKeyStoreFormat (Json.getString (ssl, Spec.Ssl.Format));
		...
		AuthorizationListener authListener = (AuthorizationListener)Server.class.getClassLoader ().loadClass (className)
