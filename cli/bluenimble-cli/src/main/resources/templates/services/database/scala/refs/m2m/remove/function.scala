...
class Remove[[Model]][[Ref]] extends AbstractApiServiceSpi {
	...
	def execute (api: Api, consumer: ApiConsumer, request: ApiRequest, response: ApiResponse): ApiOutput = {
		...
		val db: Database db 	= feature (api, classOf[Database], null, request).trx ()
		...
		List<DatabaseObject> [[refs]] = [[model]].get ("[[refs]]").asInstanceOf[List<DatabaseObject>]
		if ([[refs]] == null || [[refs]].isEmpty {
			new JsonApiOutput (
				(new JsonObject ().set ("removed", false)).asInstanceOf[JsonObject]
			)
		}
		...
		new JsonApiOutput (new JsonObject ().set ("removed", found != null).asInstanceOf[JsonObject])
	}
