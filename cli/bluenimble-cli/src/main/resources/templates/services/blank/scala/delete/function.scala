...
class Delete[[Model]] extends AbstractApiServiceSpi {

	@throws (classOf [ApiServiceExecutionException])
	def ApiOutput execute (api: Api, consumer: ApiConsumer, request: ApiRequest, response: ApiResponse): ApiOutput = {

		new JsonApiOutput (
			new JsonObject ().set ("greeting", "I'm [" + request.getService.getName + "] and I am hungry to do something that matters!")
