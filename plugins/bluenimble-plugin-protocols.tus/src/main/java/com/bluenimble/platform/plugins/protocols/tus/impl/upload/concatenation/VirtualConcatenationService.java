package com.bluenimble.platform.plugins.protocols.tus.impl.upload.concatenation;
...
public class VirtualConcatenationService implements UploadConcatenationService {
    ...
    @Override
    public InputStream getConcatenatedBytes(UploadInfo uploadInfo, String ownerKey) throws IOException, UploadNotFoundException {
        merge(uploadInfo);
        ...
        if (uploadInfo == null || uploadInfo.isUploadInProgress()) {
            ...
            return new SequenceInputStream(new UploadInputStreamEnumeration(uploads, uploadStorageService, ownerKey));
        }
    }
    ...
    private void updateUpload (UploadInfo uploadInfo) throws IOException {
        try {
            uploadStorageService.update (uploadInfo, uploadInfo.getOwnerKey ());
