...
wget --no-cache https://github.com/bluenimble/serverless/releases/download/v2.41.0/bluenimble-2.41.0-bin.tar.gz && \
  sudo tar -xvzf bluenimble-2.41.0-bin.tar.gz -C /opt/bluenimble && \
  rm -f bluenimble-2.41.0-bin.tar.gz

sudo mv /opt/bluenimble/bluenimble-2.41.0 /opt/bluenimble/platform

if [ $CLEAN = 'clean' ] ; then
	sudo rm -fr /opt/bluenimble/plugins/bluenimble-plugin-dev.playground-2.41.0
