...
wget --no-cache https://github.com/bluenimble/serverless/releases/download/v2.21.0-SNAPSHOT/bluenimble-2.21.0-SNAPSHOT-bin.tar.gz && \
  sudo tar -xvzf bluenimble-2.21.0-SNAPSHOT-bin.tar.gz -C /opt/bluenimble && \
  rm -f bluenimble-2.21.0-SNAPSHOT-bin.tar.gz

sudo mv /opt/bluenimble/bluenimble-2.21.0-SNAPSHOT /opt/bluenimble/platform

if [ $CLEAN = 'clean' ] ; then
	sudo rm -fr /opt/bluenimble/plugins/bluenimble-plugin-dev.playground-2.21.0-SNAPSHOT
