...
public class ExportDatabaseSpi extends AbstractApiServiceSpi {
	...
	public ApiOutput execute (Api api, ApiConsumer consumer, ApiRequest request,
		...
		try {
			space.feature (Database.class, provider, request).exp (
				entities, 
				space.feature (Storage.class, provider, request).root ().get (file).writer (request), 
				options, 
