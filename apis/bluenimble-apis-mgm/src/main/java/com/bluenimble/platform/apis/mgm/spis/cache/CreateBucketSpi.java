...
public class CreateBucketSpi extends AbstractApiServiceSpi {
	...
	public ApiOutput execute (Api api, final ApiConsumer consumer, ApiRequest request,
		...
		Long ttl = (Long)request.get (Spec.Ttl);
		...
		cache.create (bucket, ttl == null ? 0 : ttl);
		
		return new JsonApiOutput ((JsonObject)new JsonObject ().set (CommonOutput.Created, true));
	}
