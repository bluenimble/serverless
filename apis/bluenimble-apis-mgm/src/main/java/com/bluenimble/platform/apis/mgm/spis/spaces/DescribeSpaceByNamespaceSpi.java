...
public class DescribeSpaceByNamespaceSpi extends AbstractDescribeSpaceSpi {
	...
	public ApiOutput execute (Api api, final ApiConsumer consumer, ApiRequest request,
		...
		ApiSpace space;
		try {
			space = api.space ().space ((String)request.get (CommonSpec.Space));
		} catch (ApiAccessDeniedException e) {
			throw new ApiServiceExecutionException (e.getMessage (), e).status (ApiResponse.NOT_FOUND);
		}
		
		return describe (api, request, space);
